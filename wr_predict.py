...
# Load the model
model = load("limited_models/wr_rf.joblib")

# Load the data
wr_nn = pd.read_csv("nn_data/wr_nn.csv")
...
players = players[players["position_group"] == "WR"]
...
for index, row in players.iterrows():
    ...
    most_recent_player_data = wr_nn[wr_nn["display_name"] == row["display_name"]]
...
predictions["position"] = "WR"
pd.set_option('display.max_rows', None)
print(predictions)
predictions.to_csv("final_data/wr_final_predictions.csv", index=True)